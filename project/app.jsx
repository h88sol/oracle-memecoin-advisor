...
function App() {
  ...
  const [messages, setMessages] = useState([]); // {role:'user'|'orb', text}
  ...
  const send = useCallback(
    async (textArg) => {
      ...
      const sys = PERSONALITIES[tweaks.personality]?.prompt || PERSONALITIES.oracle.prompt;
      ...
      // Bake system prompt into first user turn (claude.complete here takes plain messages)
      if (history.length && history[0].role === "user") {
        history[0] = { role: "user", content: `${sys}\n\nUser: ${history[0].content}` };
      }

      try {
        const reply = await window.claude.complete({ messages: history });
        const clean = String(reply || "").trim() || "The signal is unclear. Try rephrasing.";
