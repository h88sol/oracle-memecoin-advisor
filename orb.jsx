...
function Orb({ state, hue = 222, size = 380, intensity = 1 }) {
  ...
  useEffect(() => {
    ...
    const draw = (t) => {
      ...
      const speakBoost =
        state === "speaking" ? 1 + 0.18 * Math.sin(elapsed * 8) : 1;
      const thinkBoost =
      ...
      // ripple rings (more active when speaking)
      const rippleCount = state === "speaking" ? 4 : state === "listening" ? 3 : 2;
      for (let i = 0; i < rippleCount; i++) {
        const phase = (elapsed * (state === "speaking" ? 0.9 : 0.5) + i / rippleCount) % 1;
        const rr = (size * 0.42) + phase * size * 0.55;
        const alpha = (1 - phase) * 0.22 * intensity;
      ...
      // speak waveform overlay — concentric pulse rings tight to surface
      if (state === "speaking") {
        for (let i = 0; i < 3; i++) {
          const ph = (elapsed * 1.4 + i * 0.33) % 1;
          const rr = coreR * (1 + ph * 0.15);
          ctx.strokeStyle = `hsla(${hue}, 100%, 85%, ${(1 - ph) * 0.6})`;
          ctx.lineWidth = 1.5;
          ctx.beginPath();
          ctx.arc(cx, cy, rr, 0, Math.PI * 2);
          ctx.stroke();
