...
function Orb({ state, hue = 222, size = 380, intensity = 1 }) {
  ...
  useEffect(() => {
    ...
    const draw = (t) => {
      ...
      // particles — only visible when actively engaged, not at idle
      if (state !== "idle") {
        const particleAlpha = state === "speaking" ? 0.45 : state === "listening" ? 0.4 : 0.25;
        particles.forEach((p) => {
          ...
          ctx.fillStyle = `hsla(${hue}, 90%, ${70 + p.z * 20}%, ${tw * particleAlpha * intensity})`;
          ctx.beginPath();
      ...
      // ripple rings — only when active, very subtle
      if (state !== "idle") {
        const rippleCount = state === "speaking" ? 2 : 2;
        const rippleSpeed = 0.45;
        for (let i = 0; i < rippleCount; i++) {
          const phase = (elapsed * rippleSpeed + i / rippleCount) % 1;
          const rr = (size * 0.42) + phase * size * 0.55;
          const alpha = (1 - phase) * 0.14 * intensity;
